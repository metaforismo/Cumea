/** Long threads carry hundreds of rows (inline screenshots included); mounting
 * all of them makes the DOM heavy even though memoized rows bail out of
 * re-renders. Only the last `TRANSCRIPT_WINDOW_SIZE` messages mount by
 * default; a pill expands the window by the same step. */
export const TRANSCRIPT_WINDOW_SIZE = 120;

export interface TranscriptWindow<T> {
  visible: T[];
  /** Messages hidden before the window — the "show earlier" count. */
  hiddenCount: number;
  /** Messages hidden after a finite search-focus window. */
  laterCount: number;
  /** The boundary actually applied after clamping; expansion steps from this,
   * not from the stored value, so a clamped window expands predictably. */
  startIndex: number;
  /** Exclusive end boundary, or the list length for a tail window. */
  endIndex: number;
}
 ...
 * thread that shrinks (search refocus replacing the list) can leave the
