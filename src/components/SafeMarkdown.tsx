...
export function safePreviewPath(value: string): string | null {
  ...
  if (
    ...
    !FILE_EXTENSION.test(candidate)
  ) return null;
  return candidate;
}

function PathButton({ path, onOpenFile, code = false, label }: {
  path: string;
  onOpenFile: (path: string) => void;
  code?: boolean;
  label?: string;
  ...
  return (
    <button
      ...
      className={cn(
        "inline break-all text-left text-accent underline decoration-accent/40 underline-offset-2 hover:decoration-accent",
...
function plainTokens(text: string, keyBase: string, onOpenFile?: (path: string) => void): ReactNode[] {
  ...
  return text.split(/(\s+)/).map((part, index) => {
    ...
    const path = safePreviewPath(candidate);
    if (!path) return part;
    return (
      <span key={`${keyBase}-path-${index}`}>
        {leading}<PathButton path={path} onOpenFile={onOpenFile} />{trailing}
      </span>
    );
  });
}
...
function inline(text: string, keyBase: string, onOpenFile?: (path: string) => void): ReactNode[] {
  ...
  while ((match = token.exec(text))) {
    if (match.index > cursor) {
      output.push(...plainTokens(text.slice(cursor, match.index), `${keyBase}-${index++}`, onOpenFile));
    }
    const value = match[0];
    if (value.startsWith("[")) {
      ...
      const path = onOpenFile ? safePreviewPath(target) : null;
      if (path) {
        output.push(<PathButton key={`${keyBase}-link-${index++}`} path={path} label={label} onOpenFile={onOpenFile!} />);
      } else if (/^https:\/\/[^\s]+$/i.test(target)) {
        output.push(
          <a key={`${keyBase}-url-${index++}`} href={target} target="_blank" rel="noreferrer noopener" className="break-all text-accent underline decoration-accent/40 underline-offset-2 hover:decoration-accent">
            {label}
          </a>,
        );
      } else {
      ...
      const path = onOpenFile ? safePreviewPath(content) : null;
      output.push(path
...
export function SafeMarkdown({ text, className, onOpenFile }: SafeMarkdownProps) {
  ...
  for (let index = 0; index < lines.length; index += 1) {
    ...
    if (heading) {
      ...
      rows.push(
        <div key={index} role="heading" aria-level={level} className={cn("mt-3 font-semibold", level === 1 ? "text-xl" : level === 2 ? "text-lg" : "text-[1em]")}>
          {inline(heading[2], `h${index}`, onOpenFile)}
        </div>,
      );
      continue;
    }
