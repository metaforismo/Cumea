...
export function boundedCanvasPlan(width: number, height: number, devicePixelRatio: number): CanvasPlan {
  ...
  if (width > MAX_CSS_EDGE || height > MAX_CSS_EDGE) {
    throw new Error("This PDF page is too large to render safely at the selected zoom");
  }
...
export function extractPageText(items: Array<{ str?: unknown; hasEOL?: unknown }>): PageText {
  ...
  for (const item of items) {
    ...
    const fragment = `${item.str}${separator}`;
...
export function PdfViewer({ token, fileName }: { token: string; fileName: string }) {
    ...
    void (async () => {
      ...
      try {
        ...
        if (Number.isFinite(announcedLength) && announcedLength > MAX_PDF_BYTES) {
          throw new Error("PDF preview is limited to 25 MB");
        }
        if (!response.ok) {
        ...
        if (!response.headers.get("content-type")?.toLowerCase().startsWith("application/pdf")) {
          throw new Error("The preview endpoint did not return a PDF");
        }
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        ...
        if (active && !isRenderCancellation(reason)) {
          setRenderError(reason instanceof Error ? reason.message : "Could not render this PDF page");
        }
      } finally {
  ...
  const status = loading
    ...
    : renderError
      ...
      : rendering
        ...
        : pdf
          ? `Page ${pageNumber} of ${pageCount} ready`
          : loadError ?? "PDF preview unavailable";
  ...
  if (loadError) {
    return (
      <div className="m-auto max-w-md rounded-2xl border border-danger/30 bg-danger/10 p-5 text-center" role="alert">
        ...
        <button type="button" onClick={() => setLoadAttempt((value) => value + 1)} className="mx-auto mt-3 flex items-center gap-1.5 rounded-lg bg-raised px-3 py-2 text-[12px] text-ink hover:bg-raised-hover">
  ...
  if (loading || !pdf) {
    return (
      <div className="m-auto flex items-center gap-2 text-[13px] text-ink-secondary" role="status">
        <Loader2 size={16} className="motion-safe:animate-spin" aria-hidden /> Preparing safe PDF preview…
      </div>
    );
  }
  ...
  return (
    <div className="flex min-h-0 flex-1 flex-col" aria-busy={rendering}>
      ...
      <div className="flex min-h-12 flex-wrap items-center justify-center gap-1.5 border-b border-hairline/50 bg-card/80 px-3 py-2" role="toolbar" aria-label="PDF page controls">
        <button type="button" onClick={() => setPageNumber((value) => Math.max(1, value - 1))} disabled={pageNumber <= 1} className="rounded-lg p-2 text-ink hover:bg-raised disabled:cursor-not-allowed disabled:opacity-35" aria-label="Previous PDF page">
          <ChevronLeft size={16} />
        </button>
        ...
        <button type="button" onClick={() => setPageNumber((value) => Math.min(pageCount, value + 1))} disabled={pageNumber >= pageCount} className="rounded-lg p-2 text-ink hover:bg-raised disabled:cursor-not-allowed disabled:opacity-35" aria-label="Next PDF page">
          <ChevronRight size={16} />
        </button>
        <span className="mx-1 h-5 w-px bg-hairline/60" aria-hidden />
        <button type="button" onClick={() => setZoom((value) => Math.max(MIN_ZOOM, value - ZOOM_STEP))} disabled={zoom <= MIN_ZOOM} className="rounded-lg p-2 text-ink hover:bg-raised disabled:cursor-not-allowed disabled:opacity-35" aria-label="Zoom out PDF">
          <ZoomOut size={16} />
        </button>
        <output className="min-w-12 text-center text-[12px] tabular-nums text-ink">{Math.round(zoom * 100)}%</output>
        <button type="button" onClick={() => setZoom((value) => Math.min(MAX_ZOOM, value + ZOOM_STEP))} disabled={zoom >= MAX_ZOOM} className="rounded-lg p-2 text-ink hover:bg-raised disabled:cursor-not-allowed disabled:opacity-35" aria-label="Zoom in PDF">
          <ZoomIn size={16} />
        </button>
        <button type="button" onClick={() => setZoom(1)} disabled={zoom === 1} className="flex items-center gap-1 rounded-lg px-2 py-2 text-[12px] text-ink hover:bg-raised disabled:cursor-not-allowed disabled:opacity-35" aria-label="Reset PDF zoom to 100 percent">
      ...
      <div className="min-h-0 flex-1 overflow-auto bg-raised/40 px-4 py-6">
        {renderError ? (
          <div className="mx-auto mt-10 max-w-md rounded-2xl border border-danger/30 bg-danger/10 p-5 text-center" role="alert">
            ...
            <button type="button" onClick={() => setRenderAttempt((value) => value + 1)} className="mx-auto mt-3 flex items-center gap-1.5 rounded-lg bg-raised px-3 py-2 text-[12px] text-ink hover:bg-raised-hover">
              <RotateCw size={13} /> Retry page
            </button>
          </div>
        ) : (
          <div className="mx-auto w-max max-w-full">
            <div className="relative overflow-hidden rounded-sm bg-white shadow-lg">
              ...
              {rendering && (
                <div className="absolute inset-0 flex items-center justify-center bg-white/80 text-[12px] text-neutral-700" role="status">
                  <Loader2 size={15} className="mr-2 motion-safe:animate-spin" aria-hidden /> Rendering page…
                </div>
              )}
            </div>
            <details className="mt-4 max-w-[48rem] rounded-xl border border-hairline/60 bg-card px-4 py-3 text-ink">
              ...
              <div className="mt-3 max-h-72 overflow-auto whitespace-pre-wrap text-[13px] leading-6 text-ink-secondary">
                {pageText
                  ? pageText.text || "No extractable text was found on this page. The page may contain only images."
                  : "Extracting accessible text for this page…"}
              </div>
