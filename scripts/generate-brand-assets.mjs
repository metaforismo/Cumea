...
const source = path.join(root, "assets/brand/iconacumea.png");

async function resize(size, destination, input = source) {
...
await resize(1024, path.join(root, "build/icon-1024.png"));
await resize(1024, path.join(root, "electron/resources/app-icon.png"));
await resize(512, path.join(root, "public/app-icon.png"));
await resize(1024, path.join(root, "apps/mobile/assets/icon.png"));

const temporary = await mkdtemp(path.join(tmpdir(), "cumea-brand-"));
try {
  const adaptiveContent = path.join(temporary, "adaptive-content.png");
  await resize(700, adaptiveContent);
  await exec("sips", [
    "--padToHeightWidth",
    "1024",
    "1024",
  ...
  for (const [size, filename] of iconset) {
    await resize(size, path.join(root, "build/icon.iconset", filename));
  }
