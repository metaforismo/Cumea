...
export function configSecretValues(config) {
    const values = [config.xai?.key, config.composio?.key, config.composio?.apiKey, config.box?.token];
...
export class SecretCatalog {
    ...
    redactText(input) {
        if (!input)
            return input;
        if (this.saturated)
            return "[content withheld: secret catalog capacity exceeded]";
        let output = input.replace(/\u001b\[[0-?]*[ -\/]*[@-~]/g, "");
        if (this.exactPattern)
            output = output.replace(this.exactPattern, REDACTED);
        for (const pattern of HIGH_CONFIDENCE_PATTERNS)
            output = output.replace(pattern, REDACTED);
        return output;
    }
    redactValue(value, depth = 0, seen = new WeakSet()) {
        if (typeof value === "string")
            return this.redactText(value);
        if (value === null || typeof value !== "object")
            return value;
        if (depth >= MAX_VALUE_DEPTH || seen.has(value))
            return "[omitted]";
        seen.add(value);
        ...
        for (const [key, item] of Object.entries(value).slice(0, MAX_COLLECTION_ENTRIES)) {
            ...
            if (key === "raw" || key === "cause")
                continue;
            output[key] = this.redactValue(item, depth + 1, seen);
        }
        return output;
    }
    safeError(error, fallback = "The operation failed.") {
