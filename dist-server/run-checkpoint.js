...
export function checkpointCursorDigest(cursor) {
    if (cursor === undefined || cursor === null)
        return null;
    const value = canonical(cursor);
    if (Buffer.byteLength(value, "utf8") > MAX_CURSOR_CANONICAL_BYTES)
        return null;
...
export function validRunCheckpoint(value, owner) {
    if (!value || typeof value !== "object" || Array.isArray(value))
        return false;
    const row = value;
    if (Object.keys(row).some((key) => !CHECKPOINT_KEYS.has(key)))
        return false;
    if (row.version !== 1 || typeof row.id !== "string" || !ID.test(row.id))
        return false;
    if (row.runId !== owner.id || row.taskId !== owner.taskId || row.botId !== owner.botId)
        return false;
    if (!["created", "turn_accepted", "session", "tool", "approval", "provider"].includes(String(row.phase)))
        return false;
    if (!["available", "unsafe", "consumed"].includes(String(row.status)))
        return false;
    if (typeof row.activeLeafId !== "string" || !ID.test(row.activeLeafId))
        return false;
    if (!row.provider || typeof row.provider !== "object" || Array.isArray(row.provider))
        return false;
    if (Object.keys(row.provider).some((key) => key !== "instanceId" && key !== "model"))
        return false;
    if (!SAFE_PROVIDER_VALUE.test(row.provider.instanceId) || !SAFE_PROVIDER_VALUE.test(row.provider.model))
        return false;
    if (!Number.isSafeInteger(row.sequence) || Number(row.sequence) < 0 || Number(row.sequence) > 1_000_000)
        return false;
    if (!Number.isSafeInteger(row.createdAt) || Number(row.createdAt) <= 0 || Number(row.createdAt) > MAX_TIMESTAMP ||
    ...
    if (row.cursor !== undefined) {
        if (!row.cursor || typeof row.cursor !== "object" || Array.isArray(row.cursor))
            return false;
        if (Object.keys(row.cursor).some((key) => key !== "instanceId" && key !== "digest"))
            return false;
        if (row.cursor.instanceId !== row.provider.instanceId || !DIGEST.test(row.cursor.digest))
            return false;
    }
    ...
    if ((row.status === "unsafe") !== unsafeReason)
        return false;
    if (row.phase === "created" && (row.status !== "unsafe" || row.unsafeReason !== "turn_not_accepted"))
        return false;
    if (row.resumedByRunId !== undefined && (typeof row.resumedByRunId !== "string" || !ID.test(row.resumedByRunId)))
        return false;
    if ((row.status === "consumed") !== (row.resumedByRunId !== undefined))
        return false;
    return true;
}
...
export function planCheckpointResume(input) {
    ...
    if (checkpoint.status === "consumed" || checkpoint.resumedByRunId)
        return { allowed: false, useProviderCursor: false, reason: "already_resumed" };
    if (input.unsafeEffects)
        return { allowed: false, useProviderCursor: false, reason: "unknown_effect" };
    if (checkpoint.status !== "available")
        return { allowed: false, useProviderCursor: false, reason: checkpoint.unsafeReason ?? "turn_not_accepted" };
    if (!input.activeLeafId || input.activeLeafId !== checkpoint.activeLeafId)
        return { allowed: false, useProviderCursor: false, reason: "branch_mismatch" };
    if (!input.providerAvailable)
        return { allowed: false, useProviderCursor: false, reason: "provider_unavailable" };
    const currentDigest = checkpointCursorDigest(input.currentCursor);
    const useProviderCursor = Boolean(input.sessionResumeCapable && checkpoint.cursor &&
