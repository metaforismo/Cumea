 ...
 */
function resolveWallClock(date, hour, minute, timezone) {
...
export function nextOccurrence(scheduleInput, after) {
    ...
    if (schedule.kind === "interval")
        return after + schedule.everyMinutes * MINUTE_MS;
    ...
    for (let offset = 0; offset <= 15; offset += 1) {
        ...
        if (schedule.kind === "weekly" && !schedule.weekdays.includes(weekday))
            continue;
...
export function projectOccurrences(scheduleInput, from, to, limit = 256) {
...
export function projectFromOccurrence(scheduleInput, first, to, limit = 256) {
    const schedule = validateSchedule(scheduleInput);
    const boundedLimit = Math.max(0, Math.min(512, Math.floor(limit)));
    if (!boundedLimit || first > to)
        return [];
