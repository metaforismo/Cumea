...
export async function sendExpoPush(targets, payload, options = {}) {
    const fetcher = options.fetcher ?? fetch;
    const result = { staleTokens: [], receipts: [] };
    for (let offset = 0; offset < targets.length; offset += MAX_BATCH) {
        ...
        for (let index = 0; index < batch.length; index += 1) {
            ...
            if (ticket?.details?.error === "DeviceNotRegistered")
                result.staleTokens.push(batch[index].token);
...
export async function checkExpoPushReceipts(receipts, options = {}) {
    if (!receipts.length)
        return [];
    const fetcher = options.fetcher ?? fetch;
    ...
    if (!response.ok)
        throw new Error(`Expo push receipt HTTP ${response.status}`);
    const body = await response.json().catch(() => null);
    const data = body?.data && typeof body.data === "object" ? body.data : {};
    return receipts.flatMap((receipt) => {
        const value = data[receipt.id];
        return value?.details?.error === "DeviceNotRegistered" ? [receipt.token] : [];
    });
}
export function scheduleExpoPushReceiptCheck(receipts, onStaleToken, options = {}) {
    if (!receipts.length)
        return;
