...
export function decodeMobileComputerPreview(encoded, mime, maxBytes = MOBILE_COMPUTER_PREVIEW_MAX_BYTES) {
    ...
    if (!base64 || base64.length > Math.ceil(maxBytes / 3) * 4 + 4 || !/^[A-Za-z0-9+/]+={0,2}$/.test(base64))
        return null;
...
function stringValue(value, max = 100_000) {
...
export function publicMobileMessage(message, visibleBotIds) {
    ...
    if (message.text !== undefined)
        safe.text = message.text;
    if (message.mime !== undefined)
        safe.mime = message.mime;
...
function publicSchedule(value) {
    ...
    if (schedule.kind === "weekly") {
        const weekdays = Array.isArray(schedule.weekdays)
            ? schedule.weekdays.filter((day) => Number.isInteger(day) && Number(day) >= 0 && Number(day) <= 6).slice(0, 7)
            : [];
        if (!weekdays.length)
            return null;
...
export function publicMobileWorkspace(value, visibleBotIds) {
    const workspace = recordValue(value) ?? {};
    const list = (key, limit = MOBILE_WORKSPACE_RECORD_LIMIT) => (Array.isArray(workspace[key]) ? workspace[key] : []).slice(-limit);
    const visible = (record) => {
        const botId = stringValue(record.botId, 100);
        if (!botId || (visibleBotIds && !visibleBotIds.has(botId)))
            return null;
        return botId;
    };
    ...
    const tasks = list("tasks").flatMap((value) => {
        ...
        if (!task || !id || !botId || !status || !TASK_STATUSES.has(status))
            return [];
    ...
    const runs = list("runs").flatMap((value) => {
        ...
        if (!run || !id || !taskId || !botId || !status || !RUN_STATUSES.has(status))
            return [];
        const steps = (Array.isArray(run.steps) ? run.steps : []).slice(-100).flatMap((value) => {
            ...
            if (!step || !stepId || !kind || !stepStatus || !STEP_KINDS.has(kind) || !STEP_STATUSES.has(stepStatus))
                return [];
            return [{
                    id: stepId,
                    kind,
        ...
        const artifacts = (Array.isArray(run.artifacts) ? run.artifacts : []).slice(-100).flatMap((value) => {
            ...
            if (!artifact || !artifactId || !kind || !ARTIFACT_KINDS.has(kind))
                return [];
            return [{
                    id: artifactId,
                    kind,
    ...
    const routines = list("routines").flatMap((value) => {
        ...
        if (!routine || !id || !botId || !name || !schedule || typeof routine.enabled !== "boolean")
            return [];
...
export function sanitizeRemoteSsePayload(payload, options = {}) {
