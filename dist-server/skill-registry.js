...
import { closeSync, constants, existsSync, fstatSync, lstatSync, mkdirSync, openSync, readFileSync, readdirSync, realpathSync, renameSync, rmSync, } from "node:fs";
...
function safeRegularFile(path, maxBytes) {
    ...
    try {
        ...
        if (!stat.isFile() || stat.size > maxBytes)
            throw httpError(409, "skill package contains an unsafe or oversized file");
...
export function validateSkillAssignment(value) {
...
export function validateSkillManifest(value, expected) {
    if (!plain(value) || !exactKeys(value, MANIFEST_KEYS))
        throw httpError(400, "skill manifest has unknown or missing fields");
    ...
    return {
        ...
        provenance: { kind: "local-unsigned", source: value.provenance.source, label },
...
function versionNumbers(version) {
    ...
    if (!match)
        throw httpError(400, "skill version must be SemVer");
...
export class SkillRegistry {
    ...
    constructor(dataDir = DATA_DIR, options = {}) {
        ...
        for (const path of [this.root, this.packagesRoot, this.stagingRoot]) {
            ...
            if (!stat.isDirectory() || stat.isSymbolicLink() || !contained(managedRoot, realpathSync(path)))
                throw httpError(409, "local skill registry path is unsafe");
        }
        this.reload();
    }
    assertVersionPath(id, version) {
        if (!SKILL_ID.test(id) || (version !== undefined && !SEMVER.test(version)))
            throw httpError(400, "skill path identity is invalid");
        for (const path of [this.root, this.packagesRoot]) {
            const stat = lstatSync(path);
            if (!stat.isDirectory() || stat.isSymbolicLink() || !contained(this.root, realpathSync(path)))
                throw httpError(409, "local skill registry path is unsafe");
        }
        const idRoot = join(this.packagesRoot, id);
        if (existsSync(idRoot)) {
            const stat = lstatSync(idRoot);
            if (!stat.isDirectory() || stat.isSymbolicLink() || !contained(this.packagesRoot, realpathSync(idRoot)))
                throw httpError(409, "local skill package path is unsafe");
        }
        ...
        if (existsSync(versionRoot)) {
            ...
            if (!stat.isDirectory() || stat.isSymbolicLink() || !contained(this.packagesRoot, realpathSync(versionRoot)))
                throw httpError(409, "local skill version path is unsafe");
        }
        return versionRoot;
    }
    reload() {
        ...
        for (const idEntry of ids) {
            if (!idEntry.isDirectory() || idEntry.isSymbolicLink() || !SKILL_ID.test(idEntry.name))
                throw httpError(409, "local skill registry contains an unsafe package path");
            ...
            if (!versions.length)
                continue;
            packageCount += 1;
            if (packageCount > SKILL_MAX_PACKAGES)
                throw httpError(409, "local skill registry exceeds its package limit");
            if (versions.length > SKILL_MAX_VERSIONS)
                throw httpError(409, "local skill version history is invalid");
            for (const versionEntry of versions) {
                if (!versionEntry.isDirectory() || versionEntry.isSymbolicLink() || !SEMVER.test(versionEntry.name))
                    throw httpError(409, "local skill registry contains an unsafe version path");
                const versionRoot = this.assertVersionPath(idEntry.name, versionEntry.name);
                if (!contained(this.packagesRoot, versionRoot))
                    throw httpError(409, "local skill path escaped its managed directory");
                ...
                if (next.has(key))
                    throw httpError(409, "local skill version is duplicated");
    ...
    has(id, version, requireEnabled = false) {
    ...
    package(id, version) {
        const manifest = this.manifests.get(`${id}@${version}`);
        if (!manifest)
            throw httpError(404, "no such local skill version");
        const versionRoot = this.assertVersionPath(id, version);
        const path = join(versionRoot, manifest.instructionFile);
        if (!contained(this.packagesRoot, realpathSync(path)))
            throw httpError(409, "local skill instruction path escaped its managed directory");
        ...
        if (digest(instructions) !== manifest.contentSha256)
            throw httpError(409, "local skill content failed integrity validation");
        return { manifest: structuredClone(manifest), instructions };
    }
    create(input, options = {}) {
        if (!plain(input))
            throw httpError(400, "skill editor input is invalid");
        const allowed = ["id", "displayName", "description", "version", "instructions", "label", "enabled"].sort();
        if (!exactKeys(input, allowed))
            throw httpError(400, "skill editor input has unknown or missing fields");
        ...
        if (!id || !version)
            throw httpError(400, "skill id or SemVer version is invalid");
        if (this.manifests.has(`${id}@${version}`))
            throw httpError(409, "that exact skill version already exists");
        const existing = this.list().filter((manifest) => manifest.id === id);
        if (!existing.length && new Set(this.list().map((manifest) => manifest.id)).size >= SKILL_MAX_PACKAGES)
            throw httpError(409, "local skill package limit reached");
        if (existing.length >= SKILL_MAX_VERSIONS)
            throw httpError(409, "local skill version history limit reached");
        if (options.requireNewer && existing.some((manifest) => compareSkillVersions(version, manifest.version) <= 0))
            throw httpError(409, "an update must use a newer SemVer version");
    ...
    import(value) {
        if (!plain(value) || !exactKeys(value, PACKAGE_KEYS))
            throw httpError(400, "skill package has unknown or missing fields");
    ...
    commit(skill) {
        ...
        if (!existsSync(idRoot))
            mkdirSync(idRoot, { recursive: false, mode: 0o700 });
        const stat = lstatSync(idRoot);
        if (!stat.isDirectory() || stat.isSymbolicLink())
            throw httpError(409, "local skill package path is unsafe");
        ...
        if (!stagingStat.isDirectory() || stagingStat.isSymbolicLink() || !contained(this.root, realpathSync(this.stagingRoot)))
            throw httpError(409, "local skill staging path is unsafe");
    ...
    setEnabled(id, version, enabled) {
        ...
        if (!contained(this.packagesRoot, realpathSync(path)))
            throw httpError(409, "local skill manifest path escaped its managed directory");
    ...
    delete(id, version) {
        ...
        try {
            ...
            if (readdirSync(idRoot).length === 0)
                rmSync(idRoot, { recursive: false });
            rmSync(quarantine, { recursive: true });
        }
        catch { }
    }
    systemPrompt(assignments) {
        if (!assignments?.length)
            return "";
        const packages = assignments.map((assignment) => this.package(assignment.id, assignment.version));
        if (packages.some((skill) => !skill.manifest.enabled))
            throw httpError(409, "an assigned local skill version is disabled");
        return [
