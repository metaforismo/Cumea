...
function issueId(path) {
    return `persistence-${createHash("sha256").update(path).digest("hex").slice(0, 20)}`;
}
function classify(error) {
    const message = error instanceof Error ? error.message : String(error);
    if (/too large|oversized/i.test(message))
        return "oversized";
    if (/version|unsupported/i.test(message))
        return "unsupported";
    if (error instanceof SyntaxError)
        return "malformed";
    if (/schema|invalid|corrupt/i.test(message))
        return "invalid_schema";
    return "unreadable";
}
export function markPersistenceIssue(path, label, error, resetValue, bytes) {
...
export function loadPersistentJson(path, options) {
    ...
    try {
        ...
        if (!stat.isFile() || stat.isSymbolicLink())
            throw new Error("invalid schema: persistence target is not a regular file");
        if (stat.size > maxBytes)
            throw new Error(`persistence file is too large (${stat.size} bytes)`);
        const fd = openSync(path, constants.O_RDONLY | (constants.O_NOFOLLOW ?? 0));
        let raw;
        try {
            const opened = fstatSync(fd);
            if (!opened.isFile() || opened.size !== stat.size)
                throw new Error("invalid schema: persistence file changed while opening");
            raw = readFileSync(fd, "utf8");
            if (Buffer.byteLength(raw, "utf8") !== opened.size)
                throw new Error("invalid schema: persistence file changed while reading");
        }
        finally {
            closeSync(fd);
        }
...
export function assertPersistenceWritable(path) {
...
export function publicPersistenceIssues() {
    return [...issues.values()].map((issue) => ({
        ...
        writesBlocked: true,
        ...(issue.recoveryPendingRestart ? { recoveryPendingRestart: true } : {}),
    }));
}
...
export function resetPersistenceIssue(id, confirmation) {
    const issue = issues.get(id);
    if (!issue)
        throw Object.assign(new Error("no such persistence issue"), { status: 404 });
    if (confirmation !== basename(issue.path)) {
        throw Object.assign(new Error(`type ${basename(issue.path)} to confirm reset`), { status: 400 });
    }
    if (issue.recoveryPendingRestart)
        throw Object.assign(new Error("this persistence reset is already waiting for an app restart"), { status: 409 });
