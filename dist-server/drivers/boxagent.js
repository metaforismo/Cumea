...
import { boxErrorMessage } from "../box.js";
...
export const BoxAgentDriver = {
    ...
    async create(input) {
        ...
        const sendTurn = async (turn) => {
            ...
            if (!token)
                throw new Error('box not configured — add {"box":{"token":"…"}} to ~/.cumea/config.json');
            if (!boxId)
        ...
        const snapshot = async () => {
            if (!token) {
                return { state: "unavailable", reason: 'no Box token — add {"box":{"token":"…"}} to ~/.cumea/config.json' };
            }
            try {
                await api("/me");
        ...
        return {
            ...
            adapter: {
                provider: DRIVER_KIND,
                capabilities: { sessionModelSwitch: "in-session", cloudComputerMcp: true },
                sendTurn,
