...
```sh
git clone https://github.com/metaforismo/Cumea && cd Cumea
pnpm install
...
| `dist-server/` | **Tracked build output.** Never hand-edit; regenerate it with `pnpm build:server` whenever server source changes. |

Data lives in `~/.cumea/` (bots, transcripts, per-thread NDJSON event logs, config with keys).
...
- **Never touch the real `~/.cumea`.** The setup file points `HOME` at a temp dir; keep it
  that way.
...
API keys are write-only: they land in `~/.cumea/config.json` via `PUT /api/config` and the API
