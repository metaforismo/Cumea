...
export interface ReplayMessage { id: string; role: "user" | "assistant"; text: string; at: number }
export interface CompactionStats {
  ...
  estimatedSubmittedTokens: number;
  selectedIdentityDigest: string;
}

export function shouldCompactContext(capabilities: { transcriptReplay?: boolean }, freshReplay: boolean): boolean {
...
function digest(ids: string[]): string { return `sha256:${createHash("sha256").update(ids.join("\0")).digest("hex")}`; }

function clipUtf8(text: string, budget: number): string {
  ...
  let head = "";
  for (const point of points) { if (bytes(head + point) > half) break; head += point; }
  let tail = "";
  for (let index = points.length - 1; index >= 0; index -= 1) {
    if (bytes(points[index] + tail) > budget - markerBytes - bytes(head)) break;
...
export function compactReplayContext(
  input: readonly ReplayMessage[],
  limits: { maxBytes?: number; maxMessages?: number } = {},
): { messages: ReplayMessage[]; stats: CompactionStats } {
  const maxBytes = limits.maxBytes ?? DEFAULT_CONTEXT_BYTES;
  const maxMessages = limits.maxMessages ?? DEFAULT_CONTEXT_MESSAGES;
  if (!Number.isSafeInteger(maxBytes) || maxBytes < 256 || !Number.isSafeInteger(maxMessages) || maxMessages < 1) throw new Error("invalid context compaction limits");
  ...
  while (recentStart < input.length && input[recentStart].role !== "user") recentStart += 1;
  for (let index = recentStart; index < input.length && selected.size < maxMessages; index += 1) selected.add(index);
  for (let index = input.length - 1; index >= 0 && selected.size < maxMessages; index -= 1) if (input[index].role === "user") selected.add(index);
  for (let index = input.length - 1; index >= 0 && selected.size < maxMessages; index -= 1) selected.add(index);
...
export function renderReplayMetadata(messages: readonly ReplayMessage[], omitted: number, maxBytes = DEFAULT_CONTEXT_BYTES): string {
