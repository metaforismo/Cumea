...
afterEach(async () => {
  ...
  await Promise.all([...directories].map(async (directory) => { directories.delete(directory); await rm(directory, { recursive: true, force: true }); }));
});
...
async function startHarness() {
  ...
  const port = await new Promise<number>((resolve, reject) => {
    ...
    child.once("exit", (code, signal) => finish(() => reject(new Error(`harness exited before listening (${code ?? signal ?? "unknown"}): ${stderr.slice(-1_000)}`))));
    const timer = setTimeout(() => finish(() => reject(new Error(`harness did not listen in time: ${stderr.slice(-1_000)}`))), 15_000);
    timer.unref?.();
  });
...
function postJson(port: number, route: string, body: unknown, headers: Record<string, string> = {}) {
  return json(port, route, { method: "POST", headers: { "content-type": "application/json", ...headers }, body: JSON.stringify(body) });
}

describe("safe file preview API", () => {
  it("keeps opaque workspace/attachment previews local-only and revokes them on bot deletion", async () => {
    ...
    await writeFile(path.join(workspace, "report.md"), "# Safe report\n\nNo HTML executes here.");
    ...
    expect(workspaceFile).toMatchObject({ name: "report.md", kind: "markdown", source: "workspace" });
    ...
    await expect(preview.json()).resolves.toMatchObject({ preview: { kind: "markdown", text: expect.stringContaining("Safe report") } });
    ...
    const upload = await fetch(`http://127.0.0.1:${port}/api/bots/${bot.id}/attachments`, {
      method: "POST",
      headers: { "content-type": "text/markdown", "x-file-name": encodeURIComponent("attachment.md"), "content-length": String(uploadBytes.length) },
      body: uploadBytes,
    });
