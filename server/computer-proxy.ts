...
async function waitForNavigation(value: string, attempts = 3): Promise<{ ok: boolean; targets: BrowserTarget[] }> {
...
const TOOLS = [
  {
    name: "screenshot",
    description:
      "See the bot's cloud computer screen when visual state is needed. Prefer browser_state for Chrome title and URL checks. Byte-identical frames are not resent.",
...
async function call(id: unknown, name: string, args: any) {
  ...
  if (name === "wait_for_navigation") {
    ...
    return text(
      ...
      result.ok
        ...
        : `navigation not verified after 3 checks; current safe URLs: ${result.targets.map((target) => target.url).join(", ") || "unavailable"}`,
      !result.ok,
    );
  }
  if (name === "observation_metrics") return text(id, JSON.stringify(observations.metrics));
  if (name === "click") {
  ...
  if (name === "press_key") {
    ...
    if (out.ok) observations.noteAction();
  ...
  if (msg.method === "initialize") {
    return send({
      ...
      result: {
        ...
        capabilities: { tools: {} },
        serverInfo: { name: "cumea-computer", version: "2" },
      },
    });
  }
