...
function mutateStoredEntry(bytes: Buffer, entryName: string, from: string, to: string): Buffer {
  ...
  for (let offset = 0; offset + 30 <= mutated.length; offset += 1) {
    ...
    if (name === entryName) {
      ...
      if (index < payloadOffset || index + Buffer.byteLength(from) > payloadOffset + compressedSize) {
        throw new Error(`missing payload text in ${entryName}`);
      }
...
describe("safe document preview", () => {
  it("classifies content by extension and magic rather than a claimed MIME", () => {
  ...
  it("rejects more than 512 central-directory entries before JSZip parses them", async () => {
    ...
    await expect(buildStructuredPreview("docx", bytes)).rejects.toMatchObject({
      status: 413,
      message: expect.stringMatching(/too many archive entries/i),
    });
  });
  ...
  it.each([
    ...
    await expect(buildStructuredPreview("docx", malformed)).rejects.toMatchObject({
      status: 415,
      message: expect.stringMatching(/central directory|multi-disk|ZIP64/i),
    });
  });

  it("rejects a high-ratio archive from metadata before inflating corrupt payload bytes", async () => {
    const repeated = "A".repeat(400_000);
    const bytes = await docx(documentXml(`<w:p><w:r><w:t>${repeated}</w:t></w:r></w:p>`), {}, "DEFLATE");
    ...
    await expect(buildStructuredPreview("docx", corrupted)).rejects.toThrow(/compression ratio/i);
  });

  it("caps actual inflation when the central directory understates an entry", async () => {
    const repeated = "A".repeat(10 * 1024 * 1024);
    const bytes = await docx(documentXml(`<w:p><w:r><w:t>${repeated}</w:t></w:r></w:p>`), {}, "DEFLATE");
    // The forged value is below the per-entry limit and below the 100x
    // metadata ratio, while the real output is larger than 8 MiB.
    const forged = patchCentralUncompressedSize(bytes, "word/document.xml", 900 * 1024);

    await expect(buildStructuredPreview("docx", forged)).rejects.toMatchObject({
      status: 413,
      message: expect.stringMatching(/too large|expands beyond/i),
    });
  });
  ...
  it("rejects invalid and unknown XML entities", async () => {
