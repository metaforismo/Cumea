...
function isManagedCredential(key: string): boolean {
...
export function stripManagedCredentials(
  ...
  for (const key of Object.keys(environment)) {
    ...
    if (isManagedCredential(normalized) && !allow.has(normalized)) delete environment[key];
  }
}
...
export function minimalProviderEnvironment(
  ...
  for (const [key, value] of Object.entries(source)) {
    const normalized = normalizedKey(key);
    if (MINIMAL_HOST_KEYS.has(normalized) || normalized.startsWith("LC_")) output[normalized] = value;
  }
  return output;
}
