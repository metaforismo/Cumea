...
export type PrivacyDestinationCategory = "local_process" | "ai_provider" | "cloud_service" | "paired_device" | "local_vm";
export type PrivacyDataCategory = "prompts" | "transcripts" | "files" | "screenshots" | "tool_args" | "notification_metadata";
...
export interface ProviderFlowSnapshot {
  instanceId: string;
  driverKind: string;
  snapshot: Pick<ProviderSnapshot, "state">;
}

export interface PrivacyInventoryInput {
  providerConfigs: InstanceConfigMap;
  providerSnapshots: ProviderFlowSnapshot[];
  boxConfigured: boolean;
  composioConfigured: boolean;
  expoPushEnabled: boolean;
  pairedMobileEnabled: boolean;
  pairedMobileAvailable: boolean;
  remoteScreenPreviewEnabled: boolean;
  localMcpAvailable: boolean;
  localMcpEnabled: boolean;
  localVmAvailable: boolean;
...
const providerData: PrivacyDataCategory[] = ["prompts", "transcripts", "files", "screenshots", "tool_args"];

function providerState(input: PrivacyInventoryInput, driverKind: string) {
...
function row(value: PrivacyFlowRow): PrivacyFlowRow {
  return value;
}

export function buildPrivacyInventory(input: PrivacyInventoryInput): { version: typeof PRIVACY_INVENTORY_VERSION; rows: PrivacyFlowRow[] } {
