...
import type { InstanceConfig, ModelCatalog, SendTurnInput } from "../../contracts.ts";
...
export interface CustomAcpConfig extends AcpConfig {
  args: string[];
  versionArgs: string[];
  models: ModelCatalog;
  authMethod?: string;
...
export interface CustomAcpProfileInput {
  label: string;
  executable: string;
  arguments: string[];
...
export interface PublicCustomAcpProfile extends CustomAcpProfileInput {
  id: string;
}
...
function record(value: unknown): Record<string, unknown> {
  if (!value || typeof value !== "object" || Array.isArray(value)) throw httpError(400, "profile must be an object");
  return value as Record<string, unknown>;
}

function boundedString(value: unknown, name: string, max = 512, allowEmpty = false): string {
...
function boundedStringArray(value: unknown, name: string, maxItems: number, fallback: string[] = []): string[] {
  if (value === undefined) return [...fallback];
  if (!Array.isArray(value) || value.length > maxItems) throw httpError(400, `invalid ${name}`);
  return value.map((item, index) => boundedString(item, `${name}[${index}]`, 1024, true));
}

function modelCatalog(value: unknown, defaultValue: unknown): ModelCatalog {
...
function validateArguments(args: string[]): void {
  for (const arg of args) {
    for (const placeholder of arg.matchAll(/\{([^{}]+)\}/g)) {
      if (placeholder[1] !== "model") throw httpError(400, `unsupported argument placeholder {${placeholder[1]}}`);
    }
  }
}

export function decodeCustomAcpProfileInput(raw: unknown): CustomAcpProfileInput {
  ...
  if (workspace && !isAbsolute(workspace)) throw httpError(400, "workspace must be an absolute path");
...
export function customAcpInstance(profile: CustomAcpProfileInput): InstanceConfig {
  return {
...
export function publicCustomAcpProfile(id: string, instance: InstanceConfig): PublicCustomAcpProfile | null {
  if (instance.driver !== CUSTOM_ACP_DRIVER_KIND) return null;
...
export function decodeCustomAcpConfig(raw: unknown): CustomAcpConfig {
  const value = record(raw);
...
function expandArguments(config: CustomAcpConfig, turn: SendTurnInput): string[] {
  const model = turn.model || config.models.default;
  return config.args.map((arg) => arg.replaceAll("{model}", model));
}

const support: AcpSupport<CustomAcpConfig> = {
  ...
  pickAuthMethod: (methods, config) => {
    ...
    if (config.authMethod) return ids.includes(config.authMethod) ? config.authMethod : null;
    return ids[0] ?? null;
  },
