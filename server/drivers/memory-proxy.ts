...
async function callTool(name: string, args: Json): Promise<{ text: string; isError?: boolean }> {
  if (name === "search_memory") {
    ...
    const documents = Array.isArray(body.documents) ? body.documents as Json[] : [];
    if (!documents.length) return { text: "No relevant durable memory found." };
    return {
      text: documents.map((document) =>
        `### ${document.path} (revision ${document.revision})\n${document.content}`,
      ).join("\n\n"),
    };
  }
  if (name === "remember") {
    ...
    if (!path || !content) return { text: "remember needs path and content.", isError: true };
...
async function handle(message: Json) {
  ...
  switch (method) {
    ...
    case "tools/call": {
      ...
      try {
        const result = await callTool(name, (params.arguments ?? {}) as Json);
