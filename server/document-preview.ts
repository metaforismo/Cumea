...
export type PreviewFileKind = "markdown" | "pdf" | "docx" | "html";
...
export function classifyPreviewFile(name: string, bytes: Buffer): { kind: PreviewFileKind; mime: string } {
  ...
  if ([".html", ".htm"].includes(extension)) {
    if (bytes.length > HTML_MAX_BYTES) throw previewError(413, "HTML preview is limited to 2 MB");
    const text = decodeUtf8(bytes, "HTML file");
    if (text.includes("\u0000")) throw previewError(415, "HTML file contains binary data");
...
export async function buildStructuredPreview(
  kind: Exclude<PreviewFileKind, "pdf" | "html">,
  bytes: Buffer,
): Promise<StructuredFilePreview> {
