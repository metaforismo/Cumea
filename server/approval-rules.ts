...
export interface ApprovalScope {
  ...
  tool: string;
  program?: string;
  autoAllowEligible: boolean;
  reason?: "invalid-tool" | "unparseable-command" | "privilege-escalation" | "obfuscated-command" | "destructive" | "sensitive";
}
...
export interface ApprovalAutoDecision {
  behavior: ApprovalRuleDecision;
  rule: ApprovalRule;
  scope: ApprovalScope;
}

export interface RememberApprovalResult {
  rule: ApprovalRule | null;
  remembered: boolean;
  warning?: string;
}

export function approvalBoundary(scope: Pick<ApprovalScope, "tool" | "program">): string {
  return scope.program ? `${scope.tool}:${scope.program}` : scope.tool;
}

export function approvalAuditText(decision: ApprovalRuleDecision, scope: Pick<ApprovalScope, "tool" | "program">): string {
 ...
 * request. */
export async function applySavedRuleDecision(
  send: () => Promise<void>,
  hooks: { accepted: () => void; rejected: () => void },
...
function canonicalTool(value: string): string | null {
...
function commandTool(tool: string): boolean {
  const bare = bareTool(tool);
  return COMMAND_TOOLS.has(bare) || /(?:^|_)(?:shell|terminal|exec|command)$/.test(bare);
}

function tokenizeSimpleCommand(input: string): string[] | null {
  const words: string[] = [];
  let current = "";
  let quote: "'" | '"' | null = null;
...
function riskReason(text: string): ApprovalScope["reason"] | undefined {
  if (DESTRUCTIVE.some((pattern) => pattern.test(text))) return "destructive";
  if (SENSITIVE.some((pattern) => pattern.test(text))) return "sensitive";
  return undefined;
}
...
export function deriveApprovalScope(rawTool: string, summary: string): ApprovalScope {
  const tool = canonicalTool(rawTool);
  if (!tool) return { key: null, tool: "unknown", autoAllowEligible: false, reason: "invalid-tool" };
  const directRisk = riskReason(`${tool}\n${summary}`);
  if (!commandTool(tool)) {
    const identityRisk = HIGH_RISK_TOOL.test(tool) ? "sensitive" as const : undefined;
  ...
  if (!words?.length) return { key: null, tool, autoAllowEligible: false, reason: "unparseable-command" };
  const index = 0;
  ...
  if (!PROGRAM.test(program)) return { key: null, tool, autoAllowEligible: false, reason: "unparseable-command" };
...
interface ApprovalRuleDocument { version: 1; rules: ApprovalRule[] }
...
export class ApprovalRuleStore {
  ...
  reload(): void {
    this.rules = loadPersistentJson<ApprovalRule[]>(this.path, {
      label: "Scoped approval rules", missing: () => [], resetValue: { version: 1, rules: [] }, maxBytes: 4 * 1024 * 1024,
      validate: (raw) => {
        const document = raw && typeof raw === "object" && !Array.isArray(raw) ? raw as { version?: unknown; rules?: unknown } : null;
        if (document?.version !== 1) throw new Error("unsupported approval-rule version");
        if (!Array.isArray(document.rules) || document.rules.length > 10_000) throw new Error("invalid approval-rule schema");
        const seen = new Set<string>();
        const counts = new Map<string, number>();
        for (const row of document.rules) {
          if (!row || typeof row !== "object") throw new Error("invalid approval rule");
          const rule = row as Partial<ApprovalRule>;
          ...
          if (!valid) throw new Error("invalid approval rule");
          seen.add(identity);
          counts.set(rule.botId!, (counts.get(rule.botId!) ?? 0) + 1);
        }
        return document.rules as ApprovalRule[];
      },
    });
  }

  list(botId: string): ApprovalRule[] {
    return this.rules.filter((rule) => rule.botId === botId).map((rule) => ({ ...rule }));
  }

  decide(botId: string, scope: ApprovalScope): ApprovalAutoDecision | null {
    if (!scope.key) return null;
    const rule = this.rules.find((candidate) => candidate.botId === botId && candidate.key === scope.key);
    if (!rule) return null;
    if (rule.decision === "allow" && !scope.autoAllowEligible) return null;
    return { behavior: rule.decision, rule: { ...rule }, scope };
  }

  remember(botId: string, scope: ApprovalScope, decision: ApprovalRuleDecision, now = Date.now()): ApprovalRule | null {
    assertPersistenceWritable(this.path);
    if (!scope.key || (decision === "allow" && !scope.autoAllowEligible)) return null;
  ...
  revoke(botId: string, ruleId: string): boolean {
    assertPersistenceWritable(this.path);
  ...
  private save(): void {
    assertPersistenceWritable(this.path);
    const document: ApprovalRuleDocument = { version: 1, rules: this.rules };
...
export function rememberApprovalAfterSettlement(
